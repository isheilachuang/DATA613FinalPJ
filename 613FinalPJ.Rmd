...
map_int(customer_list,nrow)
map_int(customer, ~length(unique(.)))
customer[, .(id, age, gender,income, education, region,purchase_frequency,purchase_amount)]
```


```{r}
customer$income<- as.numeric(as.character(customer$income))
mean_income<-mean(customer$income, na.rm = TRUE)
cat("Income Mean:", mean_income, "\n")
customer$purchase_amount<-as.numeric(as.character(customer$purchase_amount))
mean_purchase<-mean(customer$purchase_amount, na.rm = TRUE)
cat("Purchase Mean:", mean_purchase, "\n")
```
